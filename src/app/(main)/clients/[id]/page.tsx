...
export default async function ClientDetailPage({ params }: { params: { id: string } }) {
  ...
  const [resources, openTasksRes, doneTasksRes, visibleIds] = await Promise.all([
    ...
    supabase
      ...
      .limit(20)
      .then((r) => r.data ?? []),
    // Completed-tasks knowledge base. We pull title + description here
    // because the description is the actual record of what was done —
    // the page renders both so the section reads as a project history,
    // not just a list of titles. Bumped from 20 → 100 so the AI tool
    // (list_client_completed_tasks) and the human reading the page
    // both have meaningful historical context to work with.
    supabase
      .from("tasks")
