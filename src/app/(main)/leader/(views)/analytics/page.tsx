...
import { CompletionsTrendChart, StatusDistributionChart } from "@/components/AnalyticsCharts";
    ...
    <div className="space-y-5 max-w-7xl mx-auto">
      ...
      {data && (
        <>
          ...
          {/* Top-of-page charts: trendline answers "are we shipping
              more or less?", donut answers "where's the work right
              now?". Both update in the same 10s poll cycle as the
              rest of the panels. */}
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <Panel
              title="Completions · last 30 days"
              subtitle="Daily ship rate. Look for a rising tail."
              icon={<TrendingUp className="w-4 h-4 text-emerald-600" />}
              tone="blue"
              className="lg:col-span-2"
            >
              <CompletionsTrendChart data={data.completionsByDay} />
            </Panel>

            <Panel
              title="Open task status"
              subtitle="Where the team's current work is sitting."
...
function Panel({
  title, subtitle, icon, tone, children, className
}: {
  title: string;
  subtitle?: string;
  ...
  return (
    <section className={`rounded-2xl border border-slate-200/70 bg-white shadow-soft p-5${className ? ` ${className}` : ""}`}>
