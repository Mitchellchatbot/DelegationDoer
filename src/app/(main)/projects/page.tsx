...
import { projects, departments, milestones, tickets } from "@/lib/mock-data";
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        {projects.map((p) => {
          ...
          const open = tickets.filter((t) => t.projectId === p.id && t.status !== "done").length;
          return (
            <Link key={p.id} href={`/projects/${p.id}`} className="card card-hover p-4 block">
              ...
              <div className="mt-3 flex items-center justify-between text-xs text-muted">
                <div>{open} open ticket{open === 1 ? "" : "s"}</div>
