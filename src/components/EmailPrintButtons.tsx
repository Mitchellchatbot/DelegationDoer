...
import { Printer, Download, FileCode, Loader2 } from "lucide-react";
...
//   Save as HTML — self-contained .html copy (single message or whole thread)
//   Save as EML  — RFC-5322 message file that re-opens in a desktop mail client;
//                  one message → a plain .eml, a thread → a multipart/digest .eml
//                  bundling every message.
...
export function EmailPrintButtons({
  ...
  return (
    <>
      ...
      <button
        type="button"
        onClick={(e) => { e.stopPropagation(); run("html"); }}
        disabled={busy !== null}
        className={cls}
        title="Save as an HTML file"
      >
        {busy === "html" ? <Loader2 className={spin} /> : <Download className={icon} />}
        Save as HTML
      </button>
      <button
        type="button"
        onClick={(e) => { e.stopPropagation(); run("eml"); }}
        disabled={busy !== null}
        className={cls}
        title="Save as an .eml file (opens in a mail client)"
      >
        {busy === "eml" ? <Loader2 className={spin} /> : <FileCode className={icon} />}
        Save as EML
      </button>
    </>
  );
