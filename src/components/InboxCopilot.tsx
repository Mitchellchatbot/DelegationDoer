...
export function InboxCopilot({ threads, note }: { threads: InboxThread[]; note: string | null }) {
  ...
  return (
    <div className="space-y-2">
      {rows.map((r) => (
        <div key={r.id} className={"rounded-2xl border bg-white shadow-soft overflow-hidden " + (r.sent ? "border-emerald-200 opacity-70" : "border-slate-200")}>
          <button
            type="button"
            ...
            <div className="px-4 pb-4 space-y-2 border-t border-slate-100 pt-3">
              {!r.draft && !r.drafting && (
                <button
                  type="button"
                  onClick={() => draft(r.id)}
                  className="flex items-center gap-1.5 text-[12px] font-medium text-white bg-indigo-600 rounded-lg px-3 py-1.5 hover:bg-indigo-700"
                >
              ...
              {r.draft && (
                <>
                  <textarea
                    ...
                    rows={Math.min(16, Math.max(6, r.draft.split("\n").length + 1))}
                    className="w-full text-[13px] leading-relaxed rounded-xl border border-slate-200 p-3 focus:outline-none focus:border-indigo-300 resize-y"
                  />
                  <div className="flex items-center gap-2 flex-wrap">
                    <button
                      type="button"
                      onClick={() => send(r.id)}
                      disabled={r.sending || !r.draft.trim()}
                    ...
                    <input
                      value={r.instruction}
                      onChange={(e) => patch(r.id, { instruction: e.target.value })}
                      placeholder="Tweak & regenerate (e.g. 'shorter, push for a call')"
                    ...
                    <button
                      ...
                      onClick={() => draft(r.id)}
                      disabled={r.drafting}
                      className="flex items-center gap-1 text-[12px] font-medium text-indigo-600 hover:bg-indigo-50 rounded-lg px-2.5 py-1.5 disabled:opacity-50"
                    >
