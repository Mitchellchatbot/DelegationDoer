...
function DrawerPanel({
  ...
  async function save(input: { id: string; label: string; description?: string | null }) {
    setError(null);
                ...
                {forms.map((f) => (
                  <FormRow
                    ...
                    onSave={(label, description) =>
                      save({ id: f.id, label, description }).catch((e) =>
                        setError(e instanceof Error ? e.message : "save failed")
                      )
                    }
                    onDelete={() =>
    ...
    return (
      <li className="rounded-xl border border-accent/40 bg-accent/5 p-3 space-y-2">
        ...
        <div className="flex items-center justify-end gap-2 pt-1">
          <button
            type="button"
            onClick={() => { setEditing(false); setLabel(form.label); setDescription(form.description ?? ""); }}
  ...
  return (
    <li className="rounded-xl border border-slate-200 bg-white px-3 py-2.5 flex items-start justify-between gap-3 group">
      <div className="min-w-0">
        <div className="text-[13px] font-medium text-ink">{form.label}</div>
        {form.description && (
...
function NewFormForm({
  ...
  onSave: (input: { id: string; label: string; description: string | null }) => Promise<void>;
}) {
  ...
  async function go() {
    ...
    setBusy(true);
    try {
      await onSave({ id: parsedId, label: label.trim(), description: description.trim() || null });
      setLink(""); setLabel(""); setDescription("");
