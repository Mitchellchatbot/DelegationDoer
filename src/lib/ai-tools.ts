...
async function listClientCompletedTasks(input: Record<string, unknown>) {
  ...
  // Resolve a fuzzy name → exact name. We use ilike on the clients
  // table so common variants ("Hopeful Estates" vs "hopeful estates")
  // resolve. Multiple matches → tell the AI to disambiguate.
  if (clientName) {
  ...
  // sinceDays = 0 (or undefined-treated-as-default) → keep the default
  // 365-day window. Pass a number > 0 to override; pass a negative
  // number or call out the AI's "all-time" intent by passing a very
  // large value (e.g. 36500). Cleaner than a separate "no filter" flag.
  ...
  // Join assignee names so the AI can attribute work without a second
  // round-trip. Workers occasionally turn over; resolving once here is
  // cheaper than a per-task lookup.
