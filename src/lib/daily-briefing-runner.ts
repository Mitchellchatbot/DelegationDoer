...
function buildPrompts(args: {
  ...
  const system = [
    ...
    '      "text": string }           // a SHORT casual Slack DM FROM Mitchell TO that teammate — a genuine personal check-in, like a text from someone who actually cares how they are doing. Just "how are you / how is everything going / anything I can do to support you". Do NOT mention any specific project, task, client, deadline, or work status — this is about the PERSON, not their work. VARY the angle across the six (how are you doing / how are you settling in / anything you need from me / hope you are doing well / checking in) and vary the opener (do not start every one with "Hey"). Plain everyday words, contractions, 1-2 short sentences, warm and low-key. Do NOT gush, flatter, over-praise, or sound like a coach, HR, or corporate. It should read like a quick friendly text, never a status check.',
    "  ]",
    "}",
    "",
    "Rules:",
    ...
    "- The check-ins are pure personal support — how are you, anything I can help with. NEVER reference a project, task, client, deadline, or work status in them (the work data below is only for choosing WHO to reach out to and the rest of the brief, not for the message text). Relaxed, warm, human. Simple and genuine beats enthusiastic.",
