...
function getToken(): string | null {
  const t = process.env.FACEBOOK_ADS_ACCESS_TOKEN;
  return t && t.trim().length > 0 ? t.trim() : null;
}

// Generic Graph fetch — appends the access token, parses JSON, and unwraps
// Graph-shaped errors into our { ok:false } envelope.
async function graphGet<T>(path: string, params: Record<string, string> = {}): Promise<FbResult<T>> {
  const token = getToken();
  if (!token) {
    return { ok: false, error: "Missing FACEBOOK_ADS_ACCESS_TOKEN" };
  }
...
export async function listAdAccounts(): Promise<FbResult<FbAdAccount[]>> {
...
export async function getAggregatedInsights(datePreset: string = "last_30d"): Promise<FbResult<FbAggregatedInsights>> {
  const accountsRes = await listAdAccounts();
  if (!accountsRes.ok) return accountsRes;
  const accounts = accountsRes.data;
  if (accounts.length === 0) {
    return { ok: false, error: "Token has no accessible ad accounts" };
  }
  ...
  // Operator filter — restrict the dashboard to the accounts we
  // actually care about. The token can see several ad accounts (Hasan
  // Reza, Allocation Assist, etc.) but only Mitchell Price's account
  // is relevant for the outbound dashboard right now. Substring match
  // so a rename doesn't break it. If this list needs to grow, lift it
  // to an env var (FACEBOOK_ADS_ACCOUNT_FILTER="mitchell price,foo").
  const NAME_FILTER = ["mitchell price"];
  const liveAccounts = active.filter((a) =>
    NAME_FILTER.some((pat) => a.name.toLowerCase().includes(pat))
  );
  ...
  if (liveAccounts.length === 0) {
    ...
    return {
      ok: false,
      error: `No matching ad accounts (saw ${accounts.length}${note.length ? `; ${note.join(", ")}` : ""})`
    };
  }
  ...
  for (const a of liveAccounts) {
    const ins = await getAccountInsights(a, datePreset);
