...
const growthSystem = (sources: ScaleSourceFlags, rules: string) => [
  ...
  "",
  "ACCURACY — DO NOT MAKE THINGS UP (this is critical; a wrong 'fact' destroys trust in the whole brief):",
  "- NEVER invent a meeting, call, booking, email, or event. Only say a call/meeting/booking happened if it is EXPLICITLY in the data (the recent tl;dv calls list, or a facility shown at the 'booked'/'proposal' stage in the live pipeline). If it is not there, do not claim it.",
  "- NEVER conflate two differently-named companies or people. Similar names are DIFFERENT entities unless the data explicitly links them (e.g. 'The Hope Institute' and 'Hopeful Estates' are NOT the same client; a person named on one is NOT automatically tied to the other). Do not merge, cross-reference, or infer relationships between separate names.",
  "- Anyone in the client list / MRR below is an EXISTING CLIENT, not a new-acquisition prospect. Never describe a current client as a lead who 'just booked a call' or a fresh prospect. Upsell/expansion of a client must be grounded in that specific client's own data (their tl;dv call, health note, or scope), named exactly.",
  "- The outbound pipeline holds PROSPECTS (potential NEW clients), never existing clients. Keep the two worlds separate: never move a client into the prospect pipeline or a prospect into the client roster.",
  "- If two items would rely on the same shaky inference, drop both. Fewer, correct items beat more, wrong ones.",
  "",
